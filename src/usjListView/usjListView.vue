...
<script>
  ...
  export default {
    ...
    mounted () {
      this.listComponent = this.$children.filter(child => child.$el.classList.contains(LIST_CLASS))[0]
      this.detailsComponent = this.$children.filter(child => child.$el.classList.contains(DETAILS_CLASS))[0]
...
<style lang="scss" src="./usjListView.scss"></style>
